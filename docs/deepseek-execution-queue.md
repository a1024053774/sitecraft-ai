...
**② 逻辑段单独就超标**
    ...
    **逻辑段 42,992 本身已超 40KB 目标 2,032。**
    且 B5-3 一刀增重 +1,827，占超标量的 **90%**。
...
| `6d7fbb7` | **B5-3** SSE 消费三处接线 lib/sse-events | 73,940 **(+1,827)** |
| `0178e81` | **B5-4** 拆「已生成」终态视图（1/5） | 73,760 (−180) |
| `e598b2c` | **B5-5** 拆追问页视图（2/5） | 71,535 (−2,225) |
| `62e685d` | **B5-7** 拆输入页视图（3/5）+ A3/A4 升格 | 68,030 (−3,505) |
| `275006f` | **B5-8** 拆确认页视图（4/5 · 最大） | 56,005 (−12,025) |
| `d5d994b` | **B5-9** 拆生成过程视图（5/5） | **49,223 (−6,782)** |

**净值：72,113 → 49,223（−31.7%）**
