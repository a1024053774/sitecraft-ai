...
## B5 · 终态可达性（数学结论，2026-09-14 执行中）

**结论：≤40KB 不可达。** 按默认规则「目标不可达 → 收口并如实报」，继续做能做的，
不升级请示（越过红线才停）。当前**未越线**。
...
**① 只拆 UI 数学上不可能**
    ...
    5 个 step 块合计        30,014
    外壳（<div app-shell> 等）  923
    JSX 段总计              30,949

    "只拆 5 块"是 JSX 段的**上限**（5 块占 97%），拆完逻辑段**原样留下**：

    拆后 = 73,941 − 30,014 = 43,927  ❌ 仍超 2,967

**② 逻辑段单独就超标**

    B5-3 前  逻辑段 41,674  |  JSX 段 30,440
    B5-3 后  逻辑段 42,992  |  JSX 段 30,949
      ↑ 差 +1,318            ↑ 差 +509

    **逻辑段本身已超 40KB 目标**（CRLF 口径 42,992 / blob 口径约 42,146）。
    且 B5-3 一刀增重 +490（blob 口径；初版误报 +1,827 为 CRLF 口径）。

**③ 逻辑段里能搬的部分被红线挡住**

| 声明 | 字节 | 性质 |
|---|---:|---|
| `execute` | 12,272 | 业务决策 + 密集 setState |
| `analyze` | 7,289 | 同上（**29 行含 setState / 共 126 行**） |
| `recoverMissingSections` | 4,967 | 同上（**15 行含 setState / 共 86 行**） |

搬进 `lib/` 需要**把整组 state setter 透传出去**——等于把状态归属从页面挪走，
**红线①（移动状态所有者）**。

**"只搬纯逻辑"能省多少？实测约 1.5KB**（`recoverMissingSections` 的 done 字段提取
+ completion 组装），远不足以补 2,032 的缺口。**没有不越线的路径。**

### 与开单预期的差异（如实记）

- 预期「删掉 3,786 字节手写管道」→ 实际**增重 1,827**（onEvent 回调展开更长）；
- 预期「UI 占 41%、逻辑占 59%」→ 实测 JSX 30,949 / 逻辑 42,992，
  **逻辑段才是主体**，而它恰好是红线保护区。

### 次生影响

扣掉 5 块后 ≈ 43,927，**比 B4 收口时的 workspace（73,813）好得多，但仍 >40KB**。
达成 40KB 需要 `execute`/`analyze` 的控制流重写（搬状态所有权），
属 B4b 同族工作。

**处置**：继续执行 5 块拆分（它把文件从 73,941 降到 ~43,927，且**不改任何状态归属**），
拆完汇总实测数字与红线取证，一并报给用户裁决
——是接受 ~43,927 收口，还是把 `execute`/`analyze` 重写并入 B4b/B5b。

---

## B5 · 终报（2026-09-14 · 拆块收口，真实模型采集待用户）

### ① 字节曲线（以 commit blob 为准）

| 提交 | 内容 | generate/page.tsx |
|---|---|---:|
| 起点 `0846e2e` | B5 开工 | 72,113 |
| `6d7fbb7` | **B5-3** SSE 消费三处接线 lib/sse-events | 72,603 **(+490)** |
| `0178e81` | **B5-4** 拆「已生成」终态视图（1/5） | 72,428 (−175) |
| `e598b2c` | **B5-5** 拆追问页视图（2/5） | 70,253 (−2,175) |
| `62e685d` | **B5-7** 拆输入页视图（3/5）+ A3/A4 升格 | 66,845 (−3,408) |
| `275006f` | **B5-8** 拆确认页视图（4/5 · 最大） | 55,056 (−11,789) |
| `d5d994b` | **B5-9** 拆生成过程视图（5/5） | **48,377 (−6,679)** |

**净值：72,113 → 48,377（−23,736 / −32.9%）**

> ⚠️ **本表出过一次 CRLF 口径错**（2026-09-14）：初版报 49,223 / B5-3 增重 1,827，
> 全部是**工作区 CRLF 口径**；blob/LF 口径实为 48,377 / +490。
> **差 846 字节（1.7%）**，由 B5 期间新增的 846 行各带一个 `` 造成。
> 这是**第四次**同类错误（见 ⑦ 节）。已立通用规矩：**汇报里所有字节数一律用
> `git cat-file -p <sha>:<path> | wc -c`（blob/LF），工作区 `wc -c` 一律不出现在结论里。**

新组件（`components/generate/`）：
`generation-done-view` / `clarify-step-view` / `input-step-view` /
`confirm-step-view` / `generation-progress-view`

### ② 目标可达性：**≤40KB 不可达，按裁决收口于 49,223**

用户 2026-09-14 中期裁决 ①：**接受收口，不为 3.9KB 越线。**

三重障碍（已取证，见上文「B5 · 终态可达性」节）：
1. 只拆 UI 上限 30,949 → 拆完仍 43,927；
2. **逻辑段单独 42,992**（现 42,950），本身超 40KB；
3. 逻辑段主体 `execute` 12,272 / `analyze` 7,289 / `recover` 4,967
   搬它们需透传整组 setState = **挪状态所有者（红线①）**。

**状态机重写并入 B4b**（用户裁决：与跨面板状态收拢**本是一件事**）。

### ③ 一个被修正的 B4 误判（重要）

B4 收口时把 `generating` 块判为「**不可搬（触红线）**」，理由"依赖 28 符号、
含 10 字段富对象，搬它等于搬半个状态机"。

**实测证伪**：该块**零 setter、零 ref**，只有 2 个透传出口
（`onRecoverMissing` / `onEnterWorkspace`）——**是 5 块里最纯的一块**。

**当时错在把「这块 UI」与「analyze/execute 两段状态机」打包算在一起。**
真正不可搬的是后者。用户裁决 B5 时已预判此点，本次实测证实。

### ④ 核心交付：流式在场感（真机样张）

`e2e/scripts/capture-streaming-narrative.mjs`（证据脚本，不入默认套件）：

```
01  +    0ms  正在为所选模板生成内容…
02  +  853ms  正在复用模板结构，并行填充首屏和板块内容…
03  + 1552ms  正在写首屏与「关于」…
04  + 2254ms  首屏已就位，正在写「优势」「服务」…
05  + 2954ms  公司成立于 2012 年，专注工业自动化（about）
06  + 3655ms  「关于」「优势」已完成，正在写「服务」「产品」…
07  + 4352ms  从方案设计到现场调试，交付周期约 6-8 周（services、products）
08  + 5055ms  正在校验内容并保存到模板草稿…
```

**8 条文案变化 / 8 种**，截图 10 张（`test-results/b5-narrative/`，已 gitignore）。

> ⚠️ **事件源是 mock**（逐块 700ms）。它证明**前端叙事链路**，
> **不证明**真实模型下的观感。

### ⑤ 「真实模型观感采集」——**留给用户手动跑**（用户裁决 ③）

```bash
# 前置：.env.local 配好模型密钥；服务用 serve.mjs 起（它保证构建新鲜）
node e2e/scripts/serve.mjs &

# 采集（会真实调用模型一次，约 30–120 秒、按 token 计费）
SITECRAFT_REAL=1 node e2e/scripts/capture-streaming-narrative.mjs
```

- 真实模式**不注册任何 mock**，直走 `/api/sites/demo/generate` → 编排层 → 模型；
- 产物同上路径，但输出会标记「真实模型事件源」；
- **按 `@real` 惯例，成本敏感，不自动跑**；
- **这是 B5 验收的最后一项。**
...
开单前提「现状是 buffer 收完 `reverse().find()` 取最后一个事件」**不成立**。
现状的 `readSseEvents(raw)` 每收一个 chunk 就**重解析整个缓冲**再取**当前已有**的最后一个
——**本就是逐事件更新**（实测 4 个中间态全部可见）。

**后果**：红线「回退成取最后一个，断言必须红」**无法执行**——要回退成一个不存在的
实现得先写它；按附则 A1，该实验没有分辨力。`streaming-presence.spec.ts` 因此定位为
**回归网**而非负向门禁。

**切换仍做了，理由是另外三条（可取证）**：O(n²)→O(n)、envelope 去重
（路由早发 `sequence`，页面此前无 deduper）、附则 2 单源。

### ⑦ 过程中修掉的两个真实缺陷

1. **`lib/sse-events` 的 `await reader.cancel()` 会永久挂起**——
   generate 页注释里记过这个坑，lib 版本原样带着它。**红证 1005ms 挂起 → 修 → 1.4ms 通过**（`a4ec10e`）；
2. **`execute` 里 status 与 content_delta 互相覆盖**——旧实现每次取"最后一个 status
   + 最后一个 delta"，两类事件交替时后者盖掉前者。逐事件处理后各按各的语义更新
   ——**行为变准了，不是纯搬家**（已在代码注释写明）。

### ⑧ 本轮两个实战发现（已升格为军规附则）

- **附则 A3**：改源码后未触发重建的 e2e 结论**一律无效**（咬了两次：假红 + 假绿）；
- **附则 A4**：断言容器存在 ≠ 内容存在——结构回归网必须配「删子树」坏样本
  （实测删 9,833 字节子树，首版断言全绿 = 假门禁）。

### ⑨ 全套验证（收口实测）

```
tsc --noEmit   → 0 错
npm test       → tests 786 / pass 785 / fail 0 / skipped 1
playwright     → 102 passed / 8 skipped / 0 failed (3.5m)
e2e:strict     → 8 passed；T-14 探针 EMPTY 0/10
side-effects   → SIDE-EFFECT-SIGNATURE|sites=479|uploads=128|
                 captures=46|releases=11|pending-jobs=37|
                 generated-templates=15|leads=2
```

**构建新鲜度自证**（T-20 判据）：BUILD_ID 09:25:34 晚于最新源码 09:24:57 → 结论有效。

> `uploads` 120 → 128 的说明：累计来自多次 e2e 跑动里的 `asset-replace.spec.ts`
> （实测它会上传），**非 B5 代码产生**。

### ⑩ 未做（如实记）

- **真实模型采集未跑**（成本敏感，按裁决留给用户）——**B5 验收的最后一项**；
- `execute`/`analyze`/`recoverMissingSections` 状态机重写**未做**（并入 B4b）；
- generate 页**未达 ≤40KB**（49,223，按裁决收口）。

---

## 结项提速 · 纪律调整（用户 2026-09-14 裁决，**冲突以本节为准**）

### B5 · 通过收口

- **终态 48,377**（blob/LF 口径，HEAD `edcabce` 复审核实）；
- 真实模型采集：**由用户自行决定是否跑，不阻塞任何后续批次**。

### B4b · 降级

**移出关键路径，结项后新迭代再做。登记照旧**（glossary B4b 节不变）。

### T-13 · 调研压缩为一项

**只做「登录态方案两选一」**，每项**三段**（改动面 / 风险 / 回退成本），**半天量级**，
出报告**停等用户拍板**：

- **A** = 维持现状 + 网关注头；
- **B** = 前端登录态 / 凭据签发。

**红率 / T-16 / `[object Object]` 三项不单独调研**，**实施时顺带取证**。

**拍板后实施**（范围同队列 T-13）：前端带头/转发 + T-14 白屏修复 + CSP。
**每步 `tsc` + `npm test` + 相关 e2e 子集绿即 commit**；
**不再要求 3 连跑与全套 side-effects 人肉对照**；**全套只在 T-13 收尾跑一次**。

### B6 / D-2 / D-3 · 放行

**按队列执行单执行，做完直接进结项报告，中间不等复审。**

### 结项 · 交付物

一份**结项报告**：全程 commit 数 / 三入口生产可用性证据 / 遗留 T 台账。
**复审方只在此时与 T-13 实施后介入。**
...
2. **`git add` 显式列文件**；
3. **禁手抄**（B2 门禁）；
4. **触红线 a/b 仍停报**（需改 state owner / 加同步机制 / 动条件渲染，或红了定位不了）；
5. **附则 A3 / A4**（构建新鲜度 / 删子树坏样本）；
6. **M-1**（字节数一律 blob 口径）。
