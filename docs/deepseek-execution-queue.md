...
> ⚠️ **本表修正过两次，现有两版错误记录在案，见 ⑦。**
> 现行数字全部由 `git cat-file -p <sha>:<path>` 逐提交重算。

| 提交 | workspace/page.tsx | preview/route.ts | generate/page.tsx |
|---|---:|---:|---:|
| **起点 `2060216`** | **90,209** | 93,298 | 72,113 |
| ① `2a3b6ce` 商品导入/主图 | 85,611 (**−4,598**) | 93,298 | 72,113 |
...
| HEAD | 73,813 | 8,992 | 72,113 |

**净值**：workspace **−16,396**；preview/route **−84,306**（新模块 `lib/template-preview-bridge.ts` 85,804）。

**起点取 `2060216` 的依据**：`2060216` 与 `0fc4173` 的 `app/workspace/page.tsx`
**是同一个 blob**（`9e341be3b4c98f774a85dedcf6fa0c8d8fdcf1a5`，`git rev-parse` 实测），
所以 B4 起点不受中间三笔 B3 提交影响，稳定在 90,209。

**第一刀是正常减重 −4,598**：`2a3b6ce` 删掉了 `import Papa from "papaparse"` 与
`import readXlsxFile from "read-excel-file"`，解析逻辑搬到 `lib/product-import.ts`。
> **更正**：前一版终报写的「第一刀 +4,657 是真实增长、解析被内联回页面」**是错的**——
> 那是我把链序排错、拿 ④ 的 blob 当起点算出来的，据此写的自我批评不成立。已删。
...
| workspace/page.tsx | 90,209 | 73,813 | ❌ **数学不可达**（−18.2%） |
...
**同一个"字节口径"错误犯了三次**（用户 2026-09-13 判定）：**CRLF → blob → 串行。**

1. **第一次（CRLF 口径）**：中期报「91,757 → 75,206」用了工作区 CRLF 数，
   与 commit blob 差 15%+——**两个口径混用等于没数**。
2. **第二次（blob 口径但取错起点）**：终报首版把 `f24f164^`（＝④的父提交 34fe447）
   当成 B4 起点，写成 80,954，并把差出来的 +4,657 解释成"解析内联回页面"的增长。
   **真实起点是 `2060216` 的 90,209**，第一刀是正常减重 −4,598
   （`2a3b6ce` 实测删除了 `papaparse` 与 `read-excel-file` 两个 import）。
3. **第三次（串行链序）**：同一张表里 `2060216 / 823002c / 0fc4173` 三笔
   B3 时期的提交被整段漏掉，导致整张表的行序与提交链错位——
   这正是第二次错误的**根因**（起点取错是它的表现）。

**共同根因**：都是**先有数字、后找证据**。第一次是拿手边的文件量，
第二次/第三次是拿"我以为的链首"直接开算，**没有先 `git rev-parse` 确认 blob 同一性**。

4. **`cp` 还原引入 CRLF**：用 `cp` 还原基线，在 Git Bash 下会把 LF 转成 CRLF，
   导致行数 +1、锚点漂移。**改用 Python 逐字节写**（`newline=''`）。
5. **"凑行号"**：为了让门禁锚点保持 43 行，反复加/删注释行——**这是在迁就锚点，
   不是让锚点正确**。正确的 45 行就该更新锚点，已改回。

### ⑦ 新增纪律（用户 2026-09-13 裁决 2，**今后通用**）

> **「口径更正本身也要过复核——改数字的人比写数字的人更需要 blob 证据。」**

**为什么立这条**：第 2、3 次错误都发生在**"更正"这个动作里**。
更正会让人进入"我已经知道正确答案了"的心态，反而跳过取证——
而读者会因为"这是更正版"而降低警惕。**更正版本必须比首版证据更硬**。

**做法**：任何数字进汇报前，先跑一遍 `git cat-file -p <sha>:<path> | wc -c`，
并 `git rev-parse` 确认起点 blob 与上一状态的同一性。**禁止用"我印象里的链首"开算。**

**与附则 A1（假实验）/ 附则 A2（假测量）同族**：
A1 管"实验没有分辨力"，A2 管"测量对象不在可渲染 DOM 里"，
本条管**"数字没有 blob 背书"**——都是**先证明证据有效，再拿它当结论**。
...
## 并行双轨授权（用户 2026-09-13 裁决 4）

**放行两条轨并行。** 两轨**文件不重叠**，所以互不阻塞：

| 轨 | 内容 | 触及文件 | 性质 |
|---|---|---|---|
| **A · B5** | 流式体验重构 + generate 页 UI 拆分（**合并执行**） | `app/generate/page.tsx`、`components/*` | **写** |
| **B · T-13** | 生产鉴权方案 + 红率实测 + T-16 定夺 + `[object Object]` 调研 | **无**（只读 + 报告） | **只读** |

**纪律**：各自独立 commit，**互不裹挟**——不许把两轨的改动塞进同一个提交。

**B 轨的硬边界**：**方案报告等用户拍板后才实施**。本阶段**禁止写代码**，
输出物是一份**只有调查报告与方案选项**的文档。

### A 轨（B5）口径

- **裁决变更版**：流式逻辑重构 + UI 块（含 `generating` 的 28 符号透传）**合并执行**；
- **终态目标**：`app/generate/page.tsx` **≤40KB**；
- **为什么 props 面不再是问题**：控制流本来就要重写，**不存在"改行为"**，
  B4 的红线（纯搬家、零行为变更）在本批**不适用**；
- **起点**：72,113 字节（`git cat-file -p HEAD:app/generate/page.tsx | wc -c` 复核）；
- **回归基线**（拿 B4 全套六行数字当网）：

      tsc --noEmit                    → 0 错
      npm test                        → tests 785 / pass 784 / fail 0 / skipped 1
      npm run test:side-effects       → SIDE-EFFECT-SIGNATURE|sites=479|uploads=120|
                                        captures=46|releases=11|pending-jobs=37|
                                        generated-templates=15|leads=2
      npx playwright test             → 100 passed / 8 skipped / 0 failed
      npm run test:e2e:strict         → 8 passed；T-14 探针 EMPTY 0/10

- **停报条件**：**触红线 a/b 才停**（拆不动＝需改 owner/同步机制/条件渲染，或红了定位不了）。
  **目标不可达不停报**——按新增默认规则收口并如实报。
- **兜底**：若完稿仍 >40KB，**另开小批，不许 B5 中途扩面**。

### B 轨（T-13）口径

**四项调查，全部只读**：

1. **登录态方案**：给出**至少两个选项**（含队列原备选「维持现状 + 网关注头」）与取舍；
2. **红率 10 次实测**：T-14 的间歇率——**单次读数无论等多久都证明不了任何东西**，
   必须 N 次采样（附则 A1）；
3. **T-16 定夺**：两条条目（`templates:190` 证据丢失 / `generate-flow:283` 证据保留）；
