...
### ⚠️ 更要紧：3210 上那台服务器跑的是**过期构建**

实测（会话 B，2026-09-14）：

- 3210 进程命令行 = `next start -p 3210`（**裸起，不是 `e2e/scripts/serve.mjs`**）→
  绕过了 `needsBuild()`，属 **T-20 / 附则 A3** 点名的形态；
- `.next/BUILD_ID` mtime = **10:52:38**，而会话 B 的源码改动 mtime = **10:55:39**；
  → **服务器用的构建早于被测源码**。

**所以**：若 T-14 的「摘 skip 后全套 e2e」是在这台服务器上跑的，
其结论按附则 A3 **一律无效**（无论绿红）。补跑应用 `serve.mjs`（它会自动重建），
或先 `E2E_FORCE_BUILD=1`。

> 会话 B 不代为处置：这台 `next start` 不是本会话起的，命令与会话 A 既往起法一致，
> 停掉它可能打断对方正在跑的 e2e。**只登记，不动手。**
