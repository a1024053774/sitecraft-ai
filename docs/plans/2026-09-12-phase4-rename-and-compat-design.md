...
**只 append 新 ChangeSet，不构造旧条目**。
所以"写入只写新名"是**自然结果**，不需要额外做"新写时把旧历史也翻译一遍"这种危险操作。

#### ⚠️ 就地改正（2026-09-12 实施后）：下面这句原文**是错的**

> ~~历史条目里的旧 op 名会永远留着。它们不会消失，也不需要消失——读取侧能认就行。~~

**实测是反的。** `commitOperations` 读到的是**归一化之后**的 record
（归一化在 `readRecord` / `rowToRecord` 里，读出即已生效），
落盘时把**整份 `history`** 一起写回——所以：

> **一次写入会把该站点整段历史的旧名一并归一。**

举例（`tests/site-store-op-rename.test.ts` 实测）：
某站点有一条旧名历史 → 只要它被**写过一次**（任何来源的改动），
那条旧名历史在盘上也会变成新名。

**用户裁决（2026-09-12）①：接受它为特性**，不是缺陷。理由与三道护栏见
[`glossary.md` 附则 5](../glossary.md)。要点：
- 兼容期**随写入自然收敛**，不需要单独跑迁移；
- 归一化**幂等**，重复执行无副作用；
- 代价是"读取时改写落盘"——用附则 5 的定性口径处理（**落盘 op 名不具取证价值**，
  取证一律看 `generation_records`）。

**仍然成立的部分**：写入侧确实不需要兼容层（不构造旧条目）；
落盘的旧名会**自然**收敛而不是"永远留着"——比原文描述的更乐观，但机制不同。
