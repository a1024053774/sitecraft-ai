...
| T-14 | **生产 CSP 只放行桥接脚本 nonce，模板自带内联脚本被拦 → 受影响模板生产预览白屏（间歇，实测 1/20）** | **已隔离·待 T-13 修复（复验锚点 = `probe-empty-rate`）** | 红率调研移交 T-13 批，见下 |
...
**排期**：待用户定。**建议与 B6 并批评估**——B6 要往工作台加 UI，
而军规 5 禁止往巨型文件加新代码；B4b 先做，B6 才有干净落点。
