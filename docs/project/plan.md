...
- `p0-postgres-probe`：未改业务代码；真实 `postgres:16-alpine` 容器中创建/追加/读回会话通过，10 路并发追加保留 10 条且唯一用户数为 10；探针数据已精确删除。Docker 之前拉取阻塞，镜像恢复后才取得该证据。
- `p0-real-chat-probe`：隔离 site 通过 Docker web 真实 DeepSeek 请求；事实问题返回 `answer` 且不改稿，模糊“把网站改好看一点”返回 `clarify` 且不改稿；隔离 site 与会话已删除。该结果是单次探针，不是模型稳定性评测。
- `p0-backend` 当前仍未完成：会话追加在草稿 commit 成功后若失败，当前 route 可能把已修改草稿报告为 error；需独立红证据和最小一致性修正后再提交。
